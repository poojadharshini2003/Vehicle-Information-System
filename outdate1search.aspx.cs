...
public partial class outdate1search : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            try
            {
                ...
                string productindate = Request.QueryString["OUTDATE"];
                if (productindate == null)
                {
                    Response.Redirect("outdatesearch.aspx");
                }



                OleDbCommand cmd = new OleDbCommand("select * from intable where OUTDATE=productindate", con);

                OleDbParameter parameter = new OleDbParameter("@OUTDATE", productindate);
